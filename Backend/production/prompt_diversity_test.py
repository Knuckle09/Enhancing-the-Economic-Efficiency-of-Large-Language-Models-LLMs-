"""
Multi-domain prompt testing for RL readiness validation
Tests optimization across mathematical, coding, business, and creative prompts
...
class PromptDiversityTester:
    """Test system across diverse prompt categories for RL readiness"""

    def __init__(self, llm_efficiency_test=None, prompts_dir="./prompts"):
        """Initialize the tester with optional LLMEfficiencyTest instance"""
    ...
    def load_prompts_from_csv(self):
        """Load prompts from CSV files in the prompts_dir. Each file is <category>.csv"""
        test_prompts = {}
    ...
    def load_prompts(self, category):
        """Load prompts from CSV file for given category, limiting to 200 prompts"""
        try:
    ...
    def test_all_categories(self, target_reduction=0.30, min_similarity=0.80, llm="tinyllama", batch_size=10):
        """Test optimization across all prompt categories with batch processing."""
        results = {}
        overall_stats = {
            'total_prompts': 0,
    ...
    def classify_prompt(self, prompt):
        """Classify the prompt into a category based on its content using NLP patterns."""
    ...
    def route_prompt_to_llm(self, prompt, prefer_gemini=False):
        """Identify the category of the prompt and route it to the most suitable LLM.
        
        Args:
            prompt: The input prompt to route
            prefer_gemini: If True, use Gemini for generic and coding tasks when available
        """
    ...
    def process_prompt(self, prompt, target_reduction=0.3, min_similarity=0.8, max_length=512):
        """Process a single prompt and route it to the suitable LLM."""
        try:
    ...
    def analyze_results(self, results, overall_stats=None):
        """Analyze optimization results across all categories"""
        if not results:
            return {
                'overall_stats': {
                    'total_prompts': 0,
    ...
    def save_results(self, results, overall_stats):
        """Save comprehensive results for RL training data"""
...
def test_rl_readiness_multi_llm(llms=None, batch_size=None, max_samples_per_llm=100):
    """
    Enhanced multi-LLM training data collection for better RL model diversity
    
    Args:
        llms: List of LLMs to test. If None, uses all available models
        batch_size: GPU batch size for processing
        max_samples_per_llm: Maximum samples to collect per LLM (to prevent data imbalance)
    
    Returns:
        tuple: (all_results, combined_stats, llm_performance)
    """
...
def test_rl_readiness(llm="tinyllama", batch_size=None):
    """Enhanced RL readiness test with GPU batch processing"""

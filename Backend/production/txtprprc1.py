...
This module provides a class for preprocessing text data with an optimized pipeline, including:
- Removing unwanted punctuation
- Tokenizing text
- Removing stop words
- Lemmatizing words
- Finding the shortest synonym for a word
- Preserving key contextual elements
...
class TextPreprocessor:
    def __init__(self, language='english', download_resources=True):
        """Initialize the preprocessor with the optimal settings"""
    ...
    def preprocess(self, text, verbose=False, preserve_semantics=True, high_similarity=True):
        """
        Preprocess text while preserving key semantic elements
        
        Args:
            text: Input text to preprocess
            verbose: Whether to print processing information
            preserve_semantics: Whether to use semantic preservation techniques
            high_similarity: Whether to prioritize response similarity over token reduction
        """
        if verbose:
    ...
    def _is_likely_code_prompt(self, text):
        """Determine if the prompt is likely asking for code generation"""
        code_indicators = [
    ...
    def _preprocess_code_prompt(self, text, verbose=False):
        """Preprocess a code-related prompt with minimal changes to preserve semantics"""
        sentences = sent_tokenize(text)
        processed_sentences = []
    ...
    def get_shortest_synonym(self, word, min_similarity=0.8):
        """Find the shortest synonym for a word that maintains semantic similarity."""
    ...
    def aggressive_preprocess(self, text, target_reduction=0.5):
        """Aggressive preprocessing for maximum token reduction"""
    ...
    def balanced_preprocess(self, text, target_reduction=0.45, min_similarity=0.80):
        """Enhanced balanced preprocessing for guaranteed results"""
        filler_patterns = [
            r'\b(please|kindly|could you|would you|can you)\b',

"""
Local LLM Efficiency Test for Text Preprocessing

This module provides a class for testing the efficiency of preprocessed text compared to original prompts
using a local LLM. It measures token usage, response time, and evaluates response quality.
...
class NLPOptimizer:
    ...
    def _load_models(self):
        """Load all NLP models with GPU support"""
    ...
    def smart_summarization(self, text, target_reduction=0.3, min_similarity=0.8):
        """GPU-accelerated smart summarization"""
    ...
    def dependency_based_optimization(self, text, target_reduction=0.3, min_similarity=0.8):
        """Advanced dependency parsing optimization"""
    ...
    def _optimize_single_sentence(self, text, target_reduction=0.3, min_similarity=0.8):
        """Optimize within a single sentence using NLP"""
        if not self.available:
            return text
...
class LLMEfficiencyTest:
    """Tests efficiency of preprocessed text vs original with a local LLM."""

    def __init__(self, output_folder="./test_results", semantic_model_name="all-MiniLM-L6-v2", **kwargs):
        """Initialize the efficiency tester with GPU support."""
    ...
    def count_tokens(self, inputs):
        """Count tokens for a single input or a batch of inputs."""
    ...
    @contextmanager
    def timer(self):
        """Context manager for timing with GPU sync."""
        if self.device == "cuda":
            torch.cuda.synchronize()
    ...
    def generate_response(self, prompt, llm="tinyllama", max_tokens=512):
        """Generate response using Ollama or Gemini backend."""
    ...
    def get_embedding(self, text: str) -> torch.Tensor:
        """Get embedding with GPU caching."""
        cache_key = hash(text)
    ...
    def optimize_tokens(self, prompt, target_reduction=0.3, min_similarity=0.85, llm="phi", category=None):
        """Optimize tokens in prompt with category-specific optimizations"""
    ...
    def _nlp_optimize_generic(self, prompt):
        """Generic optimization using NLP."""
        try:
            if not self.nlp_optimizer.available:
                return prompt
    ...
    def run_efficiency_test(self, prompts: List[str], preprocess: bool = True, llm: str = "tinyllama") -> Dict[str, float]:
        """Run efficiency test with GPU acceleration."""
    ...
    def test_prompts(self, prompts: List[str], preprocess: bool = True, llm: str = "tinyllama") -> List[Dict[str, Any]]:
        """Test prompts with GPU batch processing."""
        results = []
    ...
    def preprocess_prompt(self, prompt: str) -> str:
        """Preprocess prompt with caching."""
    ...
    def _iterative_refinement(self, prompt, original_tokens, target_reduction, min_similarity, category=None):
        """NLP-enhanced iterative refinement"""
    ...
    def calculate_llm_similarity(self, original_response, optimized_response, llm="tinyllama"):
        """Calculate LLM-specific response similarity"""
        try:
    ...
    def _calculate_math_similarity(self, text1, text2):
        """Calculate mathematical expression similarity"""
        import re
    ...
    def progressive_optimization(self, prompt, category=None, max_attempts=3):
        """Try progressively more aggressive optimization strategies"""
        strategies = [
            (0.15, 0.85),
            (0.25, 0.82),
            (0.35, 0.80),
    ...
    def check_model_availability(self, llm="tinyllama"):
        """Check if specific LLM model is available in Ollama"""
        try:
            import requests
            import re
            response = requests.get("http://localhost:11434/api/tags", timeout=5)
